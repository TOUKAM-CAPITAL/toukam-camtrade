...
def _connect_readonly(db_path: Path) -> sqlite3.Connection:
    """Open the CAMTRADE database strictly read-only, via a file: URI.

    The dashboard and decision engine never write to this database. Some
    hosting platforms (e.g. Render's free tier) run the app as a user that
    cannot create SQLite's lock/journal file in the deploy directory, which
    makes a normal sqlite3.connect(path) fail with "unable to open database
    file" even though the file itself is readable. Opening with mode=ro
    avoids that: SQLite never attempts to create anything on disk.
    Falls back to a normal connection if the URI form is rejected for any
    reason (e.g. an unusual path), so local/Windows use is unaffected.
    """
    try:
...
class CamtradeData:
    def __init__(self, db_path: Path):
        ...
        self.conn = _connect_readonly(db_path)
